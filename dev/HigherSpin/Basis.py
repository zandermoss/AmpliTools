...
#FIXME: basis generation is broken for spin-0. Need to replace with general-spin
#module.
...
class Basis():
	def __init__(self,spin,npoint,symmetric=False):
		...
		#assert spin<2, "Only spin 0,1 implemented right now."
		#FIXME: symmetric spin-2 OK.
		#FIXME: pythonize!
		self.symbolblocks = []
	...
	def EjectMasses(self,r):
		...
		for key in r.Mdict.keys():
			masspoly = poly(1,m_1,domain='QQ(I)')
			newkey = []
			for pair in key:
				if (pair[0]==pair[1]) and (pair[0]<0):
					symbol = symbols('m_'+str(abs(pair[0])))
					massfactor = poly(symbol**2,symbol,domain='QQ(I)') 
					masspoly*=massfactor
				else:
					newkey.append(pair)
			if len(newkey)==0:
			...
			masspoly = masspoly.set_domain('QQ(I)')
			p*=masspoly
	...
	def GroupMasses(self,r,massmap):
		...
		for key in massmap:
			for mass in key:
				target_poly = poly(massmap[key],massmap[key],domain='QQ(I)')
